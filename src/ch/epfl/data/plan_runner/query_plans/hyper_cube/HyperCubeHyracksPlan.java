package ch.epfl.data.plan_runner.query_plans.hyper_cube;

import ch.epfl.data.plan_runner.components.Component;
import ch.epfl.data.plan_runner.components.DataSourceComponent;
import ch.epfl.data.plan_runner.components.hyper_cube.HyperCubeJoinComponentFactory;
...
public class HyperCubeHyracksPlan {
    private static Logger LOG = Logger.getLogger(HyperCubeHyracksPlan.class);

    private final QueryBuilder _queryBuilder = new QueryBuilder();

    private static final IntegerConversion _ic = new IntegerConversion();

    public HyperCubeHyracksPlan(String dataPath, String extension, Map conf) {
...
//		.setContentSensitiveThetaJoinWrapper(_ic);
    ...
    ArrayList<Component> cm = new ArrayList<Component>();
        cm.add(relationCustomer);
        cm.add(relationOrders);
    Component lastJoiner = HyperCubeJoinComponentFactory.createHyperCubeJoinOperator(cm, _queryBuilder).add(agg)
                .setJoinPredicate(comp)
                .setContentSensitiveThetaJoinWrapper(_ic);
